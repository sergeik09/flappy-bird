...
class Pipe(pg.sprite.Sprite):
    def __init__(self, is_top=False, height=0):
        ...
        self.image = pg.transform.scale(self.image, (pipe_size_x * TILE_SCALE, height))
        self.rect = self.image.get_rect()

        self.rect.x = SCREEN_WIDTH
        
        gap_size = 200  # Расстояние между трубами для пролета птицы
        
        if is_top:
            self.rect.top = 0  # Верхняя труба касается верхней границы
        else:
            self.rect.bottom = SCREEN_HEIGHT  # Нижняя труба касается нижней границы
...
class Game:
    ...
    def setup(self):
        ...
        self.menu_box = pg.image.load('assets/HUD Menus Contrast Blue 2.png')
        self.speed = 5

        self.speed_interval = 2500
        self.speed_timer = pg.time.get_ticks()

        self.interval = 2000  # Увеличим интервал между парами труб
        self.gap_size = 200  # Фиксированный промежуток для пролета

    def create_pipe_pair(self):
        # Вычисляем случайную высоту для верхней трубы
        min_height = 50
        max_height = SCREEN_HEIGHT - self.gap_size - min_height  # Оставляем место для нижней трубы и промежутка
        top_height = random.randint(min_height, max_height)
        
        # Вычисляем высоту нижней трубы, учитывая промежуток
        bottom_height = SCREEN_HEIGHT - (top_height + self.gap_size)
        
        # Создаем трубы с заданными высотами
    ...
    def draw(self):
        ...
        if self.mode == 'game_over':
            pg.draw.rect(self.screen, pg.Color("Orange"), (SCREEN_WIDTH//2-200, SCREEN_HEIGHT//2-150, 400, 300))
            ...
            record_text_rect = text.get_rect(center=(SCREEN_WIDTH//2, SCREEN_HEIGHT//2-65))
            self.screen.blit(record_text, record_text_rect)

            restart_text = font.render('Press LMB to continue', True, pg.Color('white'))
            restart_text_rect = text.get_rect(center=(SCREEN_WIDTH//2, SCREEN_HEIGHT//2))
            self.screen.blit(restart_text, restart_text_rect)
        elif self.mode == 'game':
            self.screen.blit(font.render(f'{self.points}', False, pg.Color('Yellow')), (SCREEN_WIDTH - 50, 20))
